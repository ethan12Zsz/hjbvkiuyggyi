...
module.exports = class DiscordProtect extends Client {
    ...
    async init(...args) {
        if (args.includes("events")) {
            ...
            evtFiles.filter(x => !x.endsWith(".disabled")).forEach((file) => {
                ...
                console.log(chalk.yellow(`${chalk.black("Event loaded !")} ${chalk.black(file)}.`));
        ...
        if (args.includes("models")) {
            ...
            this.database.authenticate().then(async () => {
                ...
                console.log(chalk.yellow(`${chalk.white("Model loaded !")} ${chalk.white("Guild.js")}.`));
                await require('../models/Member.js')(Sequelize, this)
                console.log(chalk.yellow(`${chalk.white("Model loaded !")} ${chalk.white("Member.js")}.`))
    ...
    async loadCommand(commandClass, commandName) {
        try {
            ...
            console.log(chalk.yellow(`${chalk.black("Command loaded !")} ${chalk.bold(commandName)} in the category ${chalk.bold(commandClass)}.`));
            return;
        } catch (e) {
            console.log(e);
