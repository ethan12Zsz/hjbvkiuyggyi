...
module.exports = class {
    ...
    async run(message) {
        ...
        if (message.content.match(new RegExp(`^<@!?${client.user.id}>( |)$`))) {
            ...
            return message.lineReply(message.translate.utils("tag", {
                user: message.author,
                prefix: data.prefix
            }))
        }
    ...
    let messageArray = message.content.split(" ");
        ...
        if (client.cooldown.find(c => c.id === message.author.id && c.command === cmd.name && c.guild === message.guild.id)) {
            ...
            return message.lineReply(message.translate.error(null, "cooldown", {
                cooldown: timeout
            })).then(async (m) => {
        ...
        if (cmd.ownerOnly && !client.config.owners.includes(message.author.id)) {
            return message.lineReply(message.translate.error(null, "ownerOnly", {
                owners: client.config.owners
            }))
        }
        let ss =  client.config.owners.includes(message.author.id) ||  data.whitelist.includes(message.author.id)
        if (cmd.whitelistOnly && !ss) {
            if(ss) {
            return;
            } else {
            return message.lineReply(`:x: Vous devez avoir les niveaux d'autorisation \`WhitelistOnly\` pour faire cette commande.`)
         } }
        if (cmd.userPermissions && cmd.userPermissions.length >= 1 && !client.config.owners.includes(message.author.id)) {
            if (!message.member.hasPermission(cmd.userPermissions.map(x => x.toUpperCase()))) return message.lineReply(message.translate.error(null, "userPermissions", {
        ...
        if (cmd.botPermissions && cmd.botPermissions.length >= 1 && !client.config.owners.includes(message.author.id)) {
            if (!message.guild.me.hasPermission(cmd.userPermissions.map(x => x.toUpperCase()))) return message.lineReply(message.translate.error(null, "botPermissions", {
